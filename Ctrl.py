...
if __name__ == '__main__':
    ...
    with Ctrl_graph.as_default():
        with Ctrl_sess.as_default():
            ...
            # print(Ctrl_sess.run(Controller.s_ebd))
            Controller.build_sampler(mode='train')
            # print(Ctrl_sess.run(Controller.s_ebd))
            # print(Ctrl_sess.run(Controller.branch_id))
            # print(Ctrl_sess.run(Controller.mask_input))
            # print(Ctrl_sess.run(Controller.sample_arc_seqs))
            sample_arc_seqs = Ctrl_sess.run(Controller.sample_arc_seqs)
            sample_arc_seqs = sample_arc_seqs = [[ np.array([1, 1, 1, 1, 1, 1, 1, 1]), np.array([1]), np.array([1, 1, 1, 1, 1])]]
            # print(sample_arc_seqs)
            # print(Ctrl_sess.run(Controller.sample_arc_seqs))
    child = Child()
    # child.rewards = [-0.8]
    ...
    with Ctrl_graph.as_default():
        with Ctrl_sess.as_default():
            # print(Ctrl_sess.run(Controller.s_ebd))
            Controller.upgrade_network(Ctrl_sess, child)  # init
            # print(Ctrl_sess.run(Controller.s_ebd))
    ...
    with Ctrl_graph.as_default():
        with Ctrl_sess.as_default():
            Controller.upgrade_network(Ctrl_sess, child)  # not init
    # loop
